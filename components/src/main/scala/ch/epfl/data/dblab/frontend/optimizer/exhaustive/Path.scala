 ...
 */
abstract class Path(resultParam: RelationInfo, val cost: Double) {
  /**
 ...
 */
case class BaseTablePath(override val result: RelationInfo, val tableName: String) extends Path(result, -1)
/**
 * Represents a sub-query in the query tree.
 */
case class SubqueryPath(override val result: RelationInfo, val subqueryID: Int) extends Path(result, -1)
/**
 * Represents the join of two paths.
 */
case class JoinPath(override val result: RelationInfo, val leftPath: Path, val rightPath: Path)
  extends Path(result, CostModel.joinPathCost(result, leftPath, rightPath))